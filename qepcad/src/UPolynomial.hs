...
constant :: (Num r, Eq r) => r -> UPolynomial r
uzero = Upol []
monomial k c = Upol . reverse $ unormal (k+1) [c]
uvar  = monomial 1 1
constant = Upol . normal . (:[])
...
instance (Eq r, Num r) => Num (UPolynomial r) where
    ...
    p * q = Upol . normal $ reverse pqs where
      ...
      d = if null products then -1 else maximum $ map fst products
...
qquot, remainder :: (Eq r, Fractional r) => UPolynomial r -> UPolynomial r -> UPolynomial r
remainder p q = snd $ division p q
qquot p q = fst $ division p q
